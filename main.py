...
def lr_decay(optimizer, epoch, decay_rate, init_lr):
    ...
    print(" Learning rate is set as:", lr)  # 学习率设置为
...
def train(data):
    ...
    data.show_data_summary()  # 显示数据摘要
    save_data_name = data.model_dir + ".dset"  # 保存地址
    data.save(save_data_name)
    if data.sentence_classification:  # 是否句子分类任务
        model = SentClassifier(data)
    else:
        model = SeqLabel(data)  # 序列标签
    ...
    for idx in range(data.HP_iteration):
        ...
        instance_count = 0  # 实例计数
        sample_id = 0
        sample_loss = 0  # 样品损失
        total_loss = 0  # 总损失
        right_token = 0  # 正确标签
        whole_token = 0  # 整体标签
        random.shuffle(data.train_Ids)  # 洗牌
        print("Shuffle: first input word list:", data.train_Ids[0][0])
        # # set model in train model  训练模型中的集合模型
        model.train()
...
if __name__ == '__main__':
    ...
    parser.add_argument('--status', choices=['online', 'train', 'decode'], help='update algorithm', default='decode')
    parser.add_argument('--savemodel', default="sample_data/saved_model.lstmcrf.")
    ...
    if status == 'train':
        print("MODEL: train")
        data_initialization(data)  # 数据初始化
        data.generate_instance('train')  # 数据生成实例
        data.generate_instance('dev')  # 数据生成实例
        data.generate_instance('test')  # 数据生成实例
        data.build_pretrain_emb()  # 构建预训练嵌入
        train(data)
    elif status == 'decode':
        print("MODEL: decode")
