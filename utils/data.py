...
class Data:
    def __init__(self):
        ...
        self.label_alphabet = Alphabet('label', False)
                ...
                # # if sequence labeling data format i.e. CoNLL 2003
                    ...
                    if '_' in word:
                        ...
                        for syllable in syllables:
                            if syllable != '':
                                self.char_alphabet.add(syllable)
                    else:
    ...
    def generate_instance_from_origin_str_for_syntactic_component(self, sentence, default_segment_tag='S-W'):
        """从原始字符串中生成格式化数据"""
        self.fix_alphabet()
        if default_segment_tag == 'S-Y':
            word_list = sentence.split(' ')
            # for i, char in enumerate(char_list):
            #     if is_tibetan_char(char):
            #         char_list[i] = char + '\t[OOV]0'
            #     else:
            #         char_list[i] = char + '\t[OOV]1'

            self.raw_texts, self.raw_Ids = read_origin_sentence(word_list, self.word_alphabet, self.char_alphabet,
                                                                self.feature_alphabets, self.label_alphabet,
                                                                self.number_normalized, self.MAX_SENTENCE_LENGTH,
                                                                default_segment_tag)
        else:
            self.raw_texts, self.raw_Ids = read_origin_sentence(list(sentence), self.word_alphabet, self.char_alphabet,
                                                                self.feature_alphabets, self.label_alphabet,
                                                                self.number_normalized, self.MAX_SENTENCE_LENGTH,
                                                                default_segment_tag)
        ...
        for idx in range(sent_num):
            if self.sentence_classification:
                ...
                for idy in range(sent_length):
                    ...
                    word = str(content_list[idx][0][idy])
                    label = str(predict_results[idx][idy])
                    fout.write(word + ' ' + label + '\n')
                    # fout.write(content_list[idx][0][idy].encode('utf-8') + ' ' + predict_results[idx][idy].encode('utf-8') + b'\n')
                fout.write('\n')
        fout.close()
        print("Predict %s result has been written into file. %s" % (name, self.decode_dir))

    def write_decoded_results_into_string(self, predict_results):
        ...
        for idx in range(sent_num):
            if self.sentence_classification:
                return_string = return_string + (" ".join(content_list[idx][0]) + "\t" + predict_results[idx] + '\n')
            else:
                sent_length = len(predict_results[idx])
                ...
                for idy in range(sent_length):
                    ...
                    if self.feature_num > 0:
                        return_string = return_string + content_list[idx][0][idy] + "_" + content_list[idx][1][idy][1] +\
                                        " " + str(predict_results[idx][idy]) + '  '
                    else:
                        return_string = return_string + content_list[idx][0][idy] + " " + \
                                        str(predict_results[idx][idy]) + '  '
